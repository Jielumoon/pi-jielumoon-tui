...
2026-08-13---首次 CI 实跑双矩阵失败于 footer-settings"toggle must persist to disk"：`withAgentDir` 是同步包装，async 回调刚返回 Promise 就还原 `PI_CODING_AGENT_DIR` 并删临时目录，异步测试体实际写真实 agent 目录——本地 `~/.pi/agent` 存在故误通过且每次 `npm test` 都把 `toolBackground=true`/`writeAnimation=false` 写进用户真实配置，CI 无该目录则持久化失败；另有 actions checkout/setup-node v4 的 Node 20 弃用告警---`withAgentDir` 改为 async 并 `await run(dir)`，同步用例改 `await` 调用；actions/checkout 与 actions/setup-node 升 v5；手工把被污染的用户配置 `writeAnimation` 恢复为 true，并以 mtime+内容复核测试不再触碰真实文件---修改 `tests/footer-settings.test.ts`、`.github/workflows/ci.yml`、`docs/work.md`；94/94 测试、类型检查、打包检查通过，推送后 CI 双矩阵待验证。